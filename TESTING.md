...
| `/canceldraft` | `name` (required) | Host only | Cancels a draft and deletes its channel |
...
1. `/newdraft name:test player_count:3` — bot creates `#draft-test`, opens faction pool selector there
2. Head to `#draft-test`, confirm the selector is there with all factions pre-selected
3. `/startdraft name:test` — draft begins in `#draft-test`
...
| `/newdraft` without Manage Channels permission | Draft created without dedicated channel, fallback message shown |
...
| `/startdraft` with 0 players | Ephemeral error listing all unmet requirements |
| `/startdraft` as non-host | Ephemeral error: "Only the host can start" |
| `/canceldraft` as non-host | Ephemeral error: "Only the host can cancel" |
| `/canceldraft` mid-draft | Draft cancelled cleanly, dedicated channel deleted, new draft with same name allowed |
| `/canceldraft` on a completed draft | Ephemeral error: "has already completed and cannot be cancelled" |
